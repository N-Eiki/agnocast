...
void exec_discovery_agent()
{
  ...
  const char err_msg[] = "[ERROR] [Agnocast] Failed to exec the discovery agent\n";
  ssize_t written = write(STDERR_FILENO, err_msg, sizeof(err_msg) - 1);
  (void)written;
  _exit(EXIT_FAILURE);
}
  ...
  const unsigned char * heaphook_version_ptr, const size_t heaphook_version_str_len)
{
  ...
  // self-exits when the scope empties. Failing to start it is never fatal: the
  // data plane does not depend on the observer.
