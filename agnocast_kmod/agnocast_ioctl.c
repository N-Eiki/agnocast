  ...
  uint32_t pub_shm_infos_size, union ioctl_receive_msg_args * ioctl_ret)
{
  ...
  // topic_rwsem write), so receive never races with publish; the only newly-allowed concurrency
  ...
  union ioctl_take_msg_args * ioctl_ret)
{
  ...
  //
  // A read lock still excludes the publish path (topic_rwsem write), so take never races with
